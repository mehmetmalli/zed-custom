...
impl KeymapEditor {
    ...
    fn open_edit_keybinding_modal(
        ...
        let _keystroke = keybind.keystroke_text().cloned().unwrap_or_default();
        let _arguments = keybind
            .action()
            .arguments
            .as_ref()
            .map(|arguments| arguments.text.clone());
        let _context = keybind
            .context()
            .map(|context| context.local_str().unwrap_or("global"));
        let _action = keybind.action().name;
        let _source = keybind.keybind_source().map(|source| source.name());
...
async fn save_keybinding_update(
    ...
    let (_new_keybinding, _removed_keybinding, _source) = operation.generate_telemetry();
